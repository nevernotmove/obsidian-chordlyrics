...
export default function findChunks(lines: Line[]): Chunk[][] {
    ...
    while (lineNumber < lines.length) {
        ...
        switch (line.lineType) {
            case LineType.Empty:
                group.push(new Chunk(ChunkType.Empty));
                break;
            case LineType.Header:
                group.push(new Chunk(ChunkType.Header, line.content));
                break;
            case LineType.Text:
                group.push(...getSingleLineWordChunks(line.content));
                break;
            case LineType.Chord:
                handleChordLine(group, line);
                break;
        }
        lineNumber++;
        chunks.push(group);
